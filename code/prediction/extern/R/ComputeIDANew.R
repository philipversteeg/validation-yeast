...
#                        input.dataset='data', 
ComputeIDA <- function(input='__test_medium.hdf5',
                       input.dataset='/obser/data',
  ...
  # multiprocesses
  if (processes > 1) {registerDoMC(processes)}
  ...
  if(make.new.computation || !file.exists(pcgraphfile))
  {
    ...
    if( method == 'pc' || method == 'pcstablefast' || method == 'pcstable' ) 
    {
      ...
      print(Sys.time())
      cat(' :: Starting pc with method', method, 'and', processes,'processes; be patient!\n')
      ...
      pc.fit <- pc(suffStat=list(C=cor(data),n=n),
                   ...
                   p=p,
                   alpha=alpha,
                   numCores=processes, # still doesnt seem to work... 
                   verbose=verbose)
      Adj <- pc.fit@graph
