...
# input:
#		input 				hdf5 file with 'data' element as n X p normalized data (dataframe / matrix)
#		input.dataset 		name of dataset in the file (default data)
#		lambda.low 			parameters for size of grid for cross-validation (lambda.low, lambda.high, lambda.step)
# 		lambda.high				"
#		lambda.step				"
#		fix.lambda.after 	option to fix lambda after set amount of regressions (bool)
#		round.size 			significant bits on output to constrain data size
# return:
...
ComputeRidgeCV <- function(
	...
	if (lambda.step == 'log'){ 
		# log scale 
		lambdas <- c(((1:99)/100) %o% 10^(log10(lambda.low):log10(lambda.high))) #way too finegrained....  400 lambdas.
		# lambdas <- c(((1:19)/20) %o% 10^(log10(lambda.low):log10(lambda.high)))
	} 
	...
	p <- ncol(data)
	colnames(data) <- 1:p # Needed for lazy indices later...
