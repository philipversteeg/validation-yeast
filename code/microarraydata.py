...
class MicroArrayData(object):
    ...
    def rem_mutants(self, to_remove, name=None, strict=True):
        """ Remove mutants from data.

        Args:
            to_remove:  List of genes or mutants (names) or measurements 
            (indices) to be removed.
            name:       String that is name of new instance.
            strict:     Boolean specifying if the given list must be strictly 
            removed if true, 
                        else items in the list not in the dat will be skipped

        Returns:
            A MicroArrayData instance (or inherited data class) containting new 
            data.

        Raises:
            MatchingError: if strict and the specified mutants are unavailable 
            in the data.
        """
        if type(to_remove) is int:
            to_remove=random.sample(self.mutants, to_remove)
        try: 
            to_keep = [i for i in self.mutants if i not in to_remove]
            return self.select(mut=to_keep, name=name)
        except (KeyError, ValueError):
            if strict: raise Exception('MatchingError')

    def sel_mutants(self, to_keep, name=None, strict=True):
        """ Select mutants from data.
    ...
    def rem_genes(self, to_remove, name=None, strict=True):
        ...
        Args:
            to_remove:  List of genes or genes (names) or measurements 
            (indices) to be removed.
            name:       String that is name of new instance.
            strict:     Boolean specifying if the given list must be strictly 
            removed if true, 
        ...
        Raises:
            MatchingError: if strict and the specified genes are unavailable in 
            the data.
        """
        if type(to_remove) is int:
            to_remove=random.sample(self.genes, to_remove)
        try: 
            to_keep = [i for i in self.genes if i not in to_remove]
            return self.select(gen=to_keep, name=name)
        except (KeyError, ValueError):
            if strict: raise Exception('MatchingError')

    def sel_genes(self, to_keep, name=None, strict=True):
        """ Select genes from data.
    ...
    def rem_observations(self, to_remove, name=None, strict=True):
        """ Remove observational measurements from data.
        ...
        Returns:
            A MicroArrayData instance (or inherited data class) containting 
            new data.
        ...
        try: 
            to_keep = [i for i in range(self.nobs) if i not in to_remove]
            return self.select(obs=to_keep, name=name)
        except (KeyError, ValueError):
            if strict: raise Exception('MatchingError')

    def sel_observations(self, to_keep, name=None, strict=True):
        """ Select observational measurements from data.

        Args:
            to_keep:    List of genes or mutants (names) or measurements 
            (indices) to be removed.
            name:       String that is name of new instance.
            strict:     Boolean specifying if the given list must be strictly 
            removed if true, 
        ...
        Returns:
            A MicroArrayData instance (or inherited class) containting 
            new data.
    ...
    # backwards compatible aliasses
    def sel_measurements(self, to_keep, name=None):
        return self.sel_observations(to_keep, name=None)

    def rem_measurements(self, to_keep, name=None):
        return self.rem_observations(to_keep, name=None)
...
class KemmerenOld(MicroArrayData):
    """MicroArrayData from Kemmeren (2014). Old dataset."""
    file_default = os.path.normpath('{}/kemmeren/old_data/Kemmeren.hdf5'.format(DATAPATH))

    def __init__(self, obser=None, inter=None, genes=None, mutants=None, observations=None, file=None, name='KemmerenOld'):
        super(KemmerenOld, self).__init__(obser=obser, inter=inter, genes=genes, mutants=mutants, file=file, name=name)

    @classmethod
    def load(cls, file=None, name='KemmerenOld', verbose=True):
        file = file or cls.file_default
        data = super(Kemmeren, cls).load(file=file, name=name, verbose=verbose)
        return data
